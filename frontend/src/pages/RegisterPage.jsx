...
const RegisterPage = () => {
  ...
  const getPhoneError = (phone) => {
    if (phone) {
      ...
      if (!/^254[17]\d{8}$/.test(normalized))
        return "Invalid Kenyan phone (07/1xxxxxxxx)";
    }
    return "";
  };
  ...
  const handleRegister = async (e) => {
    ...
    setLoading(true);
    try {
      const submitPhone = form.phone
        ? form.phone.replace(/[^0-9]/g, "").startsWith("0")
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-100 to-pink-100 dark:from-gray-800 dark:to-gray-900 px-4">
      <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-md">
        <h2 className="text-2xl font-bold text-orange-600 dark:text-orange-400 text-center mb-2">
          <FiStar className="inline w-6 h-6 mr-2" />
          Create an Account
        </h2>
        <p className="text-gray-600 dark:text-gray-400 text-center mb-6">
          Register to explore Desi-Etsy
        </p>
