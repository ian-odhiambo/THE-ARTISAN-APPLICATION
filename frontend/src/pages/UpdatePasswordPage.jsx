...
const UpdatePasswordPage = ({ darkMode }) => {
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center px-4 ${darkMode ? "bg-gray-900" : "bg-gradient-to-br from-orange-100 to-pink-100"}`}
    >
      <div
        className={`shadow-lg rounded-lg p-8 w-full ${darkMode ? "bg-gray-800 text-gray-100" : "bg-white"}`}
      >
        <h2
          className={`text-2xl font-bold text-center mb-4 ${darkMode ? "text-orange-400" : "text-orange-600"}`}
        >
          <FiLock className="inline w-6 h-6 mr-2" />
          Update Password
        </h2>
        ...
        {success ? (
          <p
            className={`text-center font-semibold ${darkMode ? "text-green-400" : "text-green-600"}`}
          >
