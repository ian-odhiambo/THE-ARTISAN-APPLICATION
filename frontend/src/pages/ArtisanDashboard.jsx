  ...
  return (
    <div className="flex justify-end mb-6">
      <div className="relative inline-block text-left" ref={dropdownRef}>
        ...
        >
          <FiUser className="inline w-5 h-5 mr-2" /> {user?.name}
          <svg
...
const ArtisanDashboard = ({ darkMode }) => {
    ...
    if (user?.role === "artisan") {
      const fetchProducts = async () => {
        ...
        const res = await axios.get(
          `http://localhost:5000/api/v1/products/artisan/${artisanId}`,
        );
      ...
      const fetchOrders = async () => {
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/orders/artisan/${artisanId}`,
        );
        setOrders(res.data);
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form._id) {
        await axios.put(
          `${process.env.REACT_APP_API_URL}/products/${form._id}`,
          productData,
        );
      ...
      // Refresh products
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/products/artisan/${artisanId}`,
      );
      setProducts(res.data);
    } catch (error) {
    ...
    >
      ...
      >
        <h3 className="text-2xl font-bold mb-6 flex items-center gap-2">
          {form._id ? (
            <>
              <FiEdit className="inline w-5 h-5 mr-2" /> Edit Product
            </>
          ) : (
            <>
              <FiPlus className="inline w-5 h-5 mr-2" /> Add New Product
            </>
          )}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            >
              <FiFileText className="inline w-4 h-4 mr-2" /> Product Title
            </label>
            <input
              id="title"
              name="title"
          ...
          <div>
            <label
              ...
              className={`block mb-1 font-medium ${darkMode ? "text-gray-300" : "text-gray-700"}`}
            >
              <FiTag className="inline w-4 h-4 mr-2" /> Category
            </label>
            <input
              id="category"
              name="category"
          ...
          >
            <div>
              ...
              <p className={darkMode ? "text-gray-400" : "text-gray-500"}>
                ₹{p.price}{" "}
                {p.isApproved ? (
                  <FiCheck className="inline w-4 h-4 text-green-500 ml-1" />
                ) : (
                  <FiX className="inline w-4 h-4 text-red-500 ml-1" />
                )}
              </p>
            </div>
