...
const RecommendedProducts = ({ category, currentProductId }) => {
    ...
    const fetchRecommended = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/products?category=${encodeURIComponent(category)}`,
        );
  ...
  return (
    <div className="mt-16 bg-gradient-to-r from-orange-50 to-pink-50 dark:from-gray-800 dark:to-gray-900 rounded-xl p-8">
      <h3 className="text-3xl font-bold mb-8 text-center text-orange-600 dark:text-orange-400">
        <FiStar className="inline w-6 h-6 mr-2" />
        You Might Also Like
      </h3>
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        console.error(
          "ProductDetails fetch error:",
          err.response?.data || err.message || err,
        );
        toast.error(
          `Failed to load product: ${err.response?.status || "Network Error"}`,
        );
        navigate("/");
      } finally {
        setLoading(false);
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-1 text-yellow-500 text-xl">
              {Array.from({ length: 5 }).map((_, i) => (
                <FiStar
                  key={i}
                  className={`w-4 h-4 ${i < Math.floor(product.rating || 4) ? "text-yellow-400" : "text-gray-300"}`}
                />
              ))}
            ...
            {discountPercentage > 0 ? (
              ...
              <p className="text-xl text-gray-800 font-semibold dark:text-gray-200">
                KSH{product.price}
              </p>
            )}
            ...
            <div className="flex gap-4 mt-4">
              ...
              >
                <FiShoppingBag className="inline w-4 h-4 mr-2" /> Shop Now
              </button>
            </div>
          </div>
        </div>
